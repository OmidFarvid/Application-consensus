    ...
    >
        ...
        <div class="modal-content">
          <div class="modal-body">
            <form>
              ...
              <div class="row row-no-padding" v-show="csvFile.rows.length > 0">
                <div id="csvFile">
                  <table
                    class="vuetable table table-striped table-bordered csv-table"
                  >
                    <thead>
                      <tr>
                        <th
                          v-for="(column, index) in csvFile.columns"
                          :key="index"
                        >
                    ...
                    <tbody>
                      <tr v-for="(row, index) in csvFile.rows" :key="index">
                        <td
                          v-for="(column, indexColumn) in csvFile.columns"
                          :key="indexColumn"
                        >
                          {{ row[column] }}
                        </td>
      ...
      <div slot="modal-footer" class="w-100">
        <div class="row row-no-padding width-full">
          <div class="col-md-4 col-sm-4 col-xs-12">
            <button
              v-on:click="uploadApplication"
              type="button"
              class="btn btn-success btn-block"
            >
              <i class="glyphicon glyphicon-ok"></i> Upload file
            </button>
          </div>
          <div class="col-md-3 col-sm-3 col-xs-12">
            <button
              ...
              data-dismiss="modal"
              @click="$refs.uploadApplicationsModalRef.hide()"
            >
        ...
        if (typeof FileReader !== "undefined") {
          ...
          reader.onload = function(e) {
            try {
                ...
                if (cells.length > 1) {
                  for (let j = 0; j < cells.length; j++) {
                    ...
                    if (i === 0) {
                      ...
                      row[self.csvFile.columns[j]] = cells[j];
                    }
                  }
                }
              }
              ...
              self.notifyError(
                "Some error happened when trying parse csv file: \n" + e
              );
            }
          };
          reader.readAsText(fileUpload.files[0].slice(0, 1000));
        } else {
          self.notifyError("The browser does not support HTML5.");
        }
      } else {
        self.notifyError("Please choose a valid CSV file.");
      }
    },
    uploadApplication: function() {
      if (!this.csvFile.rows.length > 0) {
        this.notifyError("Please choose a valid CSV file.");
        return;
      }
